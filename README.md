...
Proxmox VE.xml => main discovery pattern for Proxmox VE; add as an extension to OOB pattern Linux Server
...
Proxmox LXC detection.xml => pattern to detect if discovered Linux server is a container and not a full OS; add as an extension to OOB pattern Linux Server; assumes the discovery account can SSH to containers like to any other Linux
